...
def get_groq_api_key(api_key_arg: str = None) -> str:
    ...
    if not api_key:
        if 'groq_api_key' not in st.session_state:
            st.warning("To access all models and ensure good results, run Macierai Search locally!")
...
def main(api_key_arg: str = None, num_results: int = 10, max_tokens: int = 4096, default_summary_length: int = 300):
    st.set_page_config(page_title="Macierai", layout="centered", initial_sidebar_state="collapsed")
    ...
    with col1:
        if st.button("Macierai Search", key="search_button"):
            perform_search()
    with col3:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run macierai application.")
